...
(defn usr-share-tomcat7-bin-setenv-sh
  ...
  [
   (str 
     ...
     " -Duser.timezone=GMT"
     " -Xmx" Xmx "m"
     " -XX:MaxPermSize=" MaxPermSize "m"
     "\"")
   ]
  )

(def etc-tomcat7-server-xml
  ["<?xml version='1.0' encoding='utf-8'?>"
   ...
   "       daemon=\"false\" maxThreads=\"150\" minSpareThreads=\"10\"/>"
   ...
   "            pattern=\"%h %l %u %t &quot;%r&quot; %s %b %D %S\" prefix=\"localhost_access_log.\" suffix=\".txt\""
   "            resolveHosts=\"false\"/>"
   "      </Host>"
   "    </Engine>"
   "  </Service>"
...
(def etc-tomcat7-server-xml-http ; server config with http listener port 8080
  ["<?xml version='1.0' encoding='utf-8'?>"
   "<Server port='8005' shutdown='SHUTDOWN'>"
   "  <Listener className='org.apache.catalina.core.AprLifecycleListener' SSLEngine='on' />"
   "  <Listener className='org.apache.catalina.core.JasperListener' />"
